...
export const dbService = {
  ...
  async fetchVisits(userId: string): Promise<VisitRecord[]> {
    try {
      const { data, error } = await supabase.from("visits").select("*, property:properties(*)").or(`renter_id.eq.${userId},host_id.eq.${userId}`).order("scheduled_start", { ascending: true });
  ...
  async fetchApplications(): Promise<Application[]> {
    try {
      const { data, error } = await supabase.from("applications").select("*, property:properties(*)").order("created_at", { ascending: false });
      if (error || !data) return [];
      return data;
    } catch { return []; }
  },
  ...
  async createApplication(app: Partial<Application>): Promise<Application | null> {
    try {
      const { data, error } = await supabase.from("applications").insert([app]).select("*, property:properties(*)").single();
      if (error || !data) return null;
      return data;
    } catch { return null; }
  },
  ...
  async saveUserPreferences(userId: string, preferences: UserPreferences): Promise<boolean> {
    try {
      const fullPrefs: UserPreferences = { ...preferences, hasCompletedOnboarding: true, updated_at: new Date().toISOString() };
  ...
  async fetchPersonalizedProperties(userId: string, prefs?: UserPreferences | null, baseProperties: Property[] = []): Promise<Property[]> {
    ...
    return baseProperties.map((property) => {
      ...
      if (userPrefs.amenities?.length) {
        const matchedCount = userPrefs.amenities.filter((a) => property.amenities.some((pa) => pa.toLowerCase().includes(a.toLowerCase()))).length;
        score += Math.min(12, matchedCount * 3);
      }
      if (userPrefs.workplace) {
        const wpLower = userPrefs.workplace.toLowerCase();
        if (property.city.toLowerCase().includes(wpLower) || property.address_line1.toLowerCase().includes(wpLower) || (property.description || "").toLowerCase().includes(wpLower)) score += 10;
      }
