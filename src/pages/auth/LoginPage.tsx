...
export function LoginPage() {
  ...
  const continueAfterAuth = () => navigate(redirectPath || "/choose-experience", { replace: true });
    ...
    <div className="min-h-screen grid grid-cols-1 lg:grid-cols-12 bg-[#f4f6fa] font-sans text-slate-800">
      <div className="lg:col-span-6 relative p-8 sm:p-12 flex flex-col justify-between overflow-hidden min-h-[500px] lg:min-h-screen bg-slate-900">
        ...
        <div className="relative z-10">
          <Link to="/discover" className="inline-flex items-center gap-2 text-white font-bold text-xl">
            <span className="p-2 bg-indigo-600 rounded-xl"><Building2 className="h-5 w-5" /></span>
            HomiFind <span className="text-indigo-300 text-sm">AI</span>
          </Link>
        </div>
        <div className="relative z-10 max-w-md my-auto py-12">
          <h1 className="text-4xl sm:text-5xl font-black text-white leading-tight">Discover with<br /><span className="text-indigo-200">Intelligence.</span></h1>
          <p className="mt-4 text-sm text-slate-100 leading-relaxed bg-slate-950/55 backdrop-blur-md p-4 rounded-2xl border border-white/15">AI-powered property recommendations based on your budget, lifestyle, commute and real preferences.</p>
          <div className="mt-6 flex items-center gap-3 rounded-2xl bg-white/95 p-4 shadow-xl"><Sparkles className="h-6 w-6 text-amber-500" /><div><div className="text-sm font-bold">Personalized Match Score</div><div className="text-xs text-slate-500">Find homes that fit you, not just your filters.</div></div></div>
        </div>
      </div>
      ...
      <div className="lg:col-span-6 flex items-center justify-center p-6 sm:p-12">
        <div className="w-full max-w-md bg-white rounded-2xl p-8 sm:p-10 shadow-xl border border-slate-100">
          <div className="text-center mb-6"><h2 className="text-2xl font-semibold text-slate-900">Welcome Back</h2><p className="text-xs text-slate-500 mt-1">Sign in to continue your home search.</p></div>
